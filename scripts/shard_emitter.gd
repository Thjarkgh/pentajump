...
func _ready() -> void:
	...
	if get_parent() is AnimatedSprite2D:
		...
		var size = parent.get_sprite_frames().get_frame_texture("explode",0).get_size() * parent.get_scale().x
		var pos = parent.offset
		if parent.centered:
			pos -=0.5*size
		var _rect = Rect2(pos,size)
		...
		var texture = parent.get_sprite_frames().get_frame_texture("explode",0)
		for t in triangles:
...
func shatter() -> void:
	...
	for s in shards:
		s.position.x -= get_parent().get_parent().get_parent().get_position().x
		s.position.y -= get_parent().get_parent().get_parent().get_position().y
